...
rewardPerChoice = []
# vector for the calculation of the bestAction
sumVector = []
...
class Exp3Queued(threading.Thread):
   ...
   def __init__(self, name, gamma):
      ...
      #support for the frequency of the choice of action
      self.choiceFreq = [0]* numActions
      
      #support for the Distance from the last action
      self.times = [-1]*numActions
      self.D = 1
      
   #return the distance at round t and choice ch 
   def distanceLastchoice(self,t,ch):
       self.D = t-self.times[ch]
       self.times[ch] = t
       return self.D     
   
   #this is the method 'malignous' that invert the weights 
   def invertWeight(self):
       #take the max weight
       max = self.weights[np.argmax(self.weights)]
       #and overrites the weight
   ...
   def exp3Thread(self):
      ...
      for (choice, reward, est, weights) in self.exp3(numActions, self.gamma):
         ...
         #for the computation of the best action I sum the value of the respective reward at round t
         sumV = sumUpdate(sumV, t)
         #I choose the position which value is the greater
         best = np.argmax(sumV)

         bestActionCumulativeReward += rewardVector[t][best]
         ...
         #regretBound2 = math.pow(t, (2.0/3))
         
         #every 100 round write the value in the csv file
         if t % 100==0:
             csvfile.writefile(weakRegret, regretBound, weights)
             
         #invert the weights of algoritm    
         self.invertWeight()
...
def exp3threadTest(simulation_index = 0):
   ...
   for t in range(numRounds):
       #every round I generate the reward of each Action
